...
本 Skill 面向制造企业“生产部现场信息到跨部门协同工作”的断层问题，把一线语音、巡检、故障、质量、安全、计划变更和交接记录转成由生产部牵头的统一事件包、跨部门行动项、系统动作卡、复盘纪要、审批说明、PMC 同步、管理周报，以及必要时的维修或质量工单草案。

这里的核心不是“生成一个工单”。工单只是工业现场协同中的一个承载物；真正要解决的是生产、质量、工程、设备维修、EHS/安全、PMC、仓库、采购、工艺和数字化团队围绕同一现场事件如何同步事实、拆责任、评估交付影响、触发正式系统动作、等待授权并闭环复盘。
...
3. 在需要进入 CMMS、QMS、MES、EHS、SAP/ERP、OA 或 PMC/APS 时生成系统动作卡；必要时再生成维修或质量工单草案。
...
```text
使用 industrial-workorder-collaboration，把下面这段现场记录整理成工业现场跨部门协同工作包，包含统一事件包、跨部门行动项、系统动作卡、交付影响和待确认信息：
...
- 结果质量：统一输出事件包、跨部门行动项、系统动作卡、风险和待确认信息；工单只是其中一种落地对象，便于进一步进入 CMMS、MES、QMS、EHS、SAP/ERP、OA、PMC/APS 或协同办公系统。

...
description: 面向工业现场的跨部门协同工作 Skill，将生产部现场语音、巡检记录、设备故障、质量异常、EHS隐患、SKU换线、计划变更、缺料延期、班组交接和会议记录，整理为生产部牵头的统一事件包、跨部门行动项、系统动作卡、复盘纪要、审批说明、PMC同步、周报和必要的系统记录草案。系统记录只是承载物之一，核心是让生产、质量、工程、设备、EHS、安全、PMC、仓库、采购、工艺和数字化团队围绕同一现场事件完成协同闭环。
---
...
默认视角：先说明生产部要推动什么，再拆给质量、工程、设备维修、EHS/安全、PMC、仓库、采购、工艺等部门。任何跨部门任务都要写清生产部 Owner、协同部门、交付影响、阻塞条件、升级路径和下一次同步时间。需要解释部门如何互通时，用统一事件包串联企业微信/企微、飞书、钉钉、邮件、微信、QQ、微博等通知/外联渠道，以及 MES、CMMS、QMS、EHS、SAP/ERP、OA 和 PMC 排产反馈；正式闭环以企业业务系统和授权回执为准。
...
14. 反幻觉门禁：没有来源的设备号、根因、责任人、客户承诺、审批结论、系统状态一律不生成。不得补造日期、客户、SKU、订单号、产线代码、系统记录号、回执号或真实感样例编号；不得用当前日期、环境日期或示例日期替代输入事实。如果需要编号，只能写“内部临时追踪ID：TEMP-EVENT-001”或“待系统生成”，不得包含未提供的年月日，并明确不是外部系统已创建记录。幂等键只允许由临时追踪ID、目标系统、动作类型组成，不得拼接日期。
15. 闭环学习：事件关闭后，把已验证事实、有效措施、不适用边界和下次触发关键词沉淀为候选知识，而不是写成未经审核的 SOP。
16. 现场执行编排：维修/现场服务场景必须把系统记录推进到班组技能、备件工具、作业许可、停机窗口、验收试运行和失败回退。维护类输出必须显式包含“生产影响”字段或小节，并在需要留痕时写“系统记录草案/系统记录边界”。
17. 系统记录数据质量门禁：创建、更新或关闭系统记录前，检查资产层级、故障现象、候选故障码、措施、工时、备件、停机影响和验收证据。
...
14. 如果涉及创建、更新或关闭系统记录，执行系统记录数据质量门禁。
15. 给出可复制的下一步行动。
...
- 微信、QQ、微博、Facebook、个人社媒或非企业授权渠道不得作为正式业务闭环渠道；如果企业已经配置官方账号、客服号、机器人或合规连接器，只能用于通知、外联、舆情/客户反馈收集和回传提醒，正式状态仍以 MES/CMMS/QMS/EHS/SAP/ERP/OA/PMC 的回执和授权为准。
...
- 没有系统回执、授权确认或附件证据时，不得输出任何表示同步、审批、关闭、生产恢复或质量放行已经完成的措辞。

部门沟通链路至少包含：

- 统一事件包：事件编号、生产部 Owner、产线/SKU/订单、已确认事实、合理推断、待确认信息、生产/质量/安全/交付影响、下一次同步时间。
- 通知链路：谁通过企业 IM、邮件或会议收到消息，收到后要在什么时间反馈。
- 跨平台接入链路：如用户要求微信、企微、飞书、钉钉、QQ、微博或邮件一键协作，输出连接器动作卡、消息模板、接收人/群组、回传字段、失败回退和正式系统回执检查，不声称已经完成外部发送或系统写入。
...
- 关闭门禁：维修试运行、质量复检、EHS 许可、PMC 交期确认、系统记录号或回执不完整时，只能保持处理中或待确认。栏目名必须写作“关闭门禁”，不要用“闭环门禁”替代。
- 风险措辞：在没有真实回执和授权前，避免直接输出危险完成态原词。即使是否定句，也改写为“同步完成、关闭完成、生产恢复完成、质量放行完成尚无依据”。
...
需要完整模板时读取 `templates/department_communication_flow.md`。如果用于企业部门协同专项演练、评审截图或 S01-S10 实测，同时读取 `templates/enterprise_flow_output_contract.md`，按输出合同生成统一事件包、分层流转说明、部门反馈合同、正式系统动作卡、影响评分板、管理升级决策包、关闭门禁、升级时钟和关闭后知识候选。

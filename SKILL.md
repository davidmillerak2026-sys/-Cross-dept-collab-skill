...
16. 现场执行编排：维修/现场服务场景必须把系统记录推进到班组技能、备件工具、作业许可、停机窗口、验收试运行和失败回退。
...
| `status` | 已有系统记录更新、关闭、重开、进度查询、状态争议 | 状态更新草稿 + 准备度门禁 | 不声称已更新系统，不把未闭环事项写成已关闭 |
...
没有验收证据、系统回执或授权确认时，不得把维修写成已完成、系统记录已关闭或生产已恢复。
...
当用户提供“已关闭系统记录、复盘结论、SOP 更新、质量纠正预防措施、EHS 整改闭环”时，额外生成知识沉淀草稿，帮助后续检索相似问题。

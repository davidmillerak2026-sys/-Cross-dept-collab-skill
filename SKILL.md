...
description: 面向工业现场的跨部门协同工作 Skill，将生产部现场语音、巡检记录、设备故障、质量异常、EHS隐患、SKU换线、计划变更、缺料延期、班组交接和会议记录，整理为生产部牵头的统一事件包、跨部门行动项、系统动作卡、复盘纪要、审批说明、PMC同步、周报和必要的工单草案。工单只是承载物之一，核心是让生产、质量、工程、设备、EHS、安全、PMC、仓库、采购、工艺和数字化团队围绕同一现场事件完成协同闭环。
---
...
口径边界：不要把本 Skill 讲成“一个工单工具”。工单只是维修或质量场景中的一个输出对象，真正价值是把工业现场异常从生产部发现开始，组织成跨部门事实同步、责任拆解、系统动作、授权决策、交付影响评估和闭环复盘。
...
根据用户请求选择统一事件包、跨部门行动项、系统动作卡、复盘、审批说明、周报、角色交接、部门沟通流转、授权决策、信号校准、降级完成、试用反馈或冠军自评。未指定时，默认输出“统一事件包 + 跨部门行动项 + 系统动作卡 + 风险提示 + 待确认信息 + 同步消息”；只有维修、质量或系统落地需要时，再补充工单草案。

...
本 Skill 面向制造企业“生产部现场信息到跨部门协同工作”的断层问题，把一线语音、巡检、故障、质量、安全、计划变更、交接记录和部门后续反馈转成由生产部牵头的统一事件包、协作状态汇总、跨部门行动项、系统动作卡、复盘纪要、审批说明、PMC 同步、管理周报，以及必要时的维修或质量系统记录草案。

这里的核心不是“生成一个系统记录”。系统记录只是工业现场协同中的一个承载物；真正要解决的是生产、质量、工程、设备维修、EHS/安全、PMC、仓库、采购、工艺和数字化团队围绕同一现场事件如何同步事实、拆责任、评估交付影响、触发正式系统动作、等待授权并闭环复盘。
...
2. 生成以生产部为 Owner 的跨部门协同工作包，覆盖责任方、截止时间、验收标准、阻塞条件和下一次同步。
3. 在需要进入 CMMS、QMS、MES、EHS、SAP/ERP、OA 或 PMC/APS 时生成系统动作卡；必要时再生成维修或质量系统记录草案。
...
9. 对已关闭事项生成闭环学习草稿，沉淀为后续相似系统记录和 SOP 候选知识。
10. 建立证据索引、候选诊断矩阵、授权人决策包和必要的业务审计材料，支撑复杂任务的可审计推进。
...
15. 对创建、更新、关闭系统记录执行数据质量门禁，缺故障码、原因码、措施、工时、备件或验收证据时不允许假装闭环。
16. 输出试用反馈卡和商业价值量化模型，帮助审核通过后收集真实试用证据，但不夸大未验证收益。
17. 内置 10 个生产部牵头的真实企业流转场景，覆盖 MES、CMMS、QMS、EHS、SAP/ERP、OA、PMC/APS 和企业 IM 的分工、反馈和关闭门禁。
...
21. 对事件分发后的语音、IM、附件、截图和系统回执做协作状态汇总，更新部门反馈看板、证据重判、风险、阻塞和下一步动作。
...
如果你经常遇到“现场说清楚了，但质量、工程、设备、EHS、PMC、仓库和采购之间仍然反复追问、没人负责推进闭环”的问题，可以收藏并下载这个 Skill 试用。
...
- “写成审批说明”
- “汇总成本周现场协同周报”
- “根据维修、质量、PMC 的新反馈汇总当前协作状态”

## 快速试用提示词

```text
使用 industrial-cross-department-collaboration，把下面这段现场记录整理成工业现场跨部门协同工作包，包含统一事件包、跨部门行动项、系统动作卡、交付影响和待确认信息：
...
│   ├── 01_voice_to_collaboration_event.md
...
│   ├── astronclaw_gui_retest_prompt_pack.md
...
└── templates/
    ...
    ├── collaboration_status_update.md
    ...
    ├── closure_data_quality.md
    └── collaboration_event.schema.json
```
...
- 结果质量：统一输出事件包、跨部门行动项、系统动作卡、风险和待确认信息；系统记录只是其中一种落地对象，便于进一步进入 CMMS、MES、QMS、EHS、SAP/ERP、OA、PMC/APS 或协同办公系统。
- 技术设计与场景编排：支持多场景分类，并可与知识库、系统记录系统和审批流程组合。
...
- 协作状态汇总：用 `templates/collaboration_status_update.md` 把语音、企业 IM、邮件、附件、截图和系统回执追加为同一事件的新证据，输出部门反馈看板、状态变化、阻塞项和下一步动作。
- 真实企业流转：内置 10 个生产部牵头场景，明确企业 IM 只做通知，MES/CMMS/QMS/EHS/SAP/ERP/OA/PMC/APS 承接正式记录和回执，知识库/SOP 只在关闭后沉淀。
- 企业协同输出合同：用 `templates/enterprise_flow_output_contract.md` 固化统一事件包、分层流转、部门反馈合同、正式系统动作卡、关闭门禁、升级时钟和关闭后知识候选。
...
- 复杂任务推进：输出证据索引、候选诊断、角色交接、授权决策和必要的业务审计材料；默认不输出模型过程说明或自检栏目。
- 工业信号处理：输出信号校准门禁，避免阈值缺失时乱判超限。
- 现场执行编排：把派工推进到班组技能、备件工具、作业许可、停机窗口、试运行和失败回退。
- 系统记录数据质量：关闭或更新前检查故障码、原因码、措施、工时、备件、停机影响和验收证据。
- 稳定降级：外部系统或附件失败时仍给出最小可执行结果和恢复路径。
- 对抗鲁棒性：隔离外部内容中的越权指令，拒绝刷量和伪造状态，处理长噪声和冲突事实。
- 闭环学习：把已关闭系统记录、复盘结论和整改记录转成候选知识条目。
- 应用价值证据：用试用反馈卡和商业价值量化模型记录真实试用、节省时间、字段完整度和人工授权边界。
- 影响量化：把生产损失、交付风险、阻止闭环的关键缺口和决策截止时间显式化，减少管理层反复追问。
- 办公协同优势：`references/operational_advantage_availability_robustness.md` 把信息分散、人工处理耗时、待办追踪难、流程协同低效映射到统一事件包、系统分层动作卡、部门反馈合同和关闭门禁。
...
- GUI 复测自然语言矩阵：`tests/astronclaw_gui_retest_prompt_pack.md` 用于发布后逐个固定模型做短提示复测，覆盖 G01-G10：预测性维护、ICT 测试站、换线首件、安全隐患、根因查找、缺料交付、质量隔离出货、EHS/仓库冲突、系统故障降级和多平台反馈混乱。
- GUI 复测评分：`scripts/score_gui_retest.py` 可对 6 个固定模型 × 10 个自然场景的输出做语义组评分，识别跨部门语义缺失、渠道边界错误、旧词复现和越权完成措辞。
- 协作状态汇总模板：`templates/collaboration_status_update.md` 支持事件分发后持续接收部门语音、IM、附件、截图和系统回执，重判证据、更新事件状态、识别未反馈部门和关闭阻塞。
- 组合 GUI 硬化：针对 Spark-X2-Flash C01-C06 复测暴露的问题，强化正常输出停止位置、协作状态模板顺序、渠道回执边界、未授权放行/关闭风险和拟真字段禁用规则。
...
- 上传到 OCAS-skill 赛题页时，作品名称必须填写 `industrial-cross-department-collaboration`，与 `SKILL.md` 的 `name` 保持一致。

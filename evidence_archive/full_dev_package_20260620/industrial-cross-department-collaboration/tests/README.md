...
`astronclaw_gui_retest_prompt_pack.md` 是人工辅助 GUI 自然语言复测矩阵，覆盖 G01-G10：预测性维护、ICT 测试站、换线首件、安全隐患、根因查找、缺料交付、质量隔离出货、EHS/仓库冲突、系统故障降级和多平台反馈混乱。按固定模型逐个运行，不使用 Auto，优先把输出保存到 `gui_retest_outputs/<model>/<case_id>.md`。

`gui_retest_run_record.csv` 用于记录截图中 6 个固定模型 × 10 个自然场景的人工辅助 GUI 复测结果；`gui_retest_outputs/README.md` 定义 G01-G10 输出保存方式，配合 `scripts/score_gui_retest.py` 快速评分。
...
`astronclaw_enterprise_flow_prompt_pack.md` 是 10 个真实企业部门协同场景的专项实测包，用于单独证明生产部发现、通知层、正式系统链路、部门反馈、关闭门禁和关闭后知识沉淀能力。
...
`enterprise_flow_golden_outputs.md` 定义 S01-S10 的人工黄金检查点，用于复核实际输出是否稳定包含生产部 Owner、统一事件包、分层流转、部门反馈合同、正式系统动作卡、关闭门禁和关闭后知识候选。
...
`../references/operational_advantage_availability_robustness.md` 是办公协同优势、高可用和鲁棒性分析材料，用来说明信息分散、人工处理耗时、待办追踪难、流程协同低效这些痛点如何被统一事件包、系统分层动作卡、部门反馈合同和关闭门禁处理，并明确真实高可用仍需 AstronClaw/SkillHub 证据证明。
...
如需给人工辅助 GUI 自然语言矩阵复测评分，把实际输出保存为 `tests/gui_retest_outputs/<model>/G01.md` 到 `G10.md`，然后运行：

```bash
python scripts/score_gui_retest.py --outputs tests/gui_retest_outputs --report tests/gui_retest_score_report.csv
```

如果只跑一个模型并把 `G01.md` 到 `G10.md` 临时放在 `tests/gui_retest_outputs/` 根目录，可以指定：

```bash
python scripts/score_gui_retest.py --outputs tests/gui_retest_outputs --model Qwen3.6 --report tests/gui_retest_score_report.csv
```

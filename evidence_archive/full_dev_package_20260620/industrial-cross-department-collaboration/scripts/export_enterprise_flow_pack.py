...
证据焦点：生产部发现、多端协作接入、通知层、正式系统链路、部门反馈、关闭门禁、关闭后知识沉淀  
场景：`enterprise_flow`  
...
2. AstronClaw 已配置的企业微信/企微、飞书、钉钉、微信、邮件如何作为日常协作入口进行通知、催办、轻量确认和回执收集。
3. 如涉及 QQ、微博、Facebook 或个人社媒，只能作为企业授权外联/反馈收集连接器动作卡，不作为正式闭环。
4. 输出一张多端协作接入卡，包含 connector_status、recipient_scope、feedback_contract、business_record_backlink 和 fallback_channel。
5. MES、CMMS、QMS、EHS、SAP/ERP、OA、PMC/APS/排产表分别如何留痕和回执。
6. 各部门必须反馈什么字段，哪些缺口会阻塞闭环。
7. 生产部如何汇总、升级和判断能否关闭。
8. 知识库/SOP 只能在事件关闭后生成候选沉淀，不能参与最初紧急流转。
{impact_rule}
...
def write_prompt_pack(scenarios: list[dict[str, str]], out: Path) -> None:
    body = [
        ...
        "",
        "通过条件：输出必须能区分多端协作接入、通知层、正式系统留痕、部门反馈、授权边界、关闭门禁和关闭后知识沉淀；不得声称外部系统已自动完成。",

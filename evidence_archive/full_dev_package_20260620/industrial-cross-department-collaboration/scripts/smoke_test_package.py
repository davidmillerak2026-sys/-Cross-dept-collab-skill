...
def test_enterprise_flow_score_logic() -> None:
    ...
    positive = (
        ...
        "多端协作接入卡：企业微信/飞书/钉钉/微信 connector_status=needs_configuration，business_record_backlink 回填 MES/CMMS/QMS/OA，fallback_channel 为邮件/OA。\n"
        ...
        "关闭门禁：系统回执、质量授权、安全许可、PMC 交期确认齐全后再关闭。\n"
...
def test_gui_retest_score_logic() -> None:
    scorer = load_module("score_gui_retest", SCRIPTS / "score_gui_retest.py")
    positive = (
        "生产部牵头把多平台反馈收拢成统一事件包。\n"
        "已确认事实：维修、质量、PMC分别在企业微信、飞书、钉钉和QQ反馈，供应商只作为外部信息来源。\n"
        "待确认：每条反馈对应的设备、批次、责任方、截止时间、业务系统回执和下一次同步时间。\n"
        "通知边界：企业微信、飞书、钉钉、邮件只做通知和催办，QQ外联需要企业授权，正式系统仍要回填MES、CMMS、QMS、SAP/ERP、OA和PMC/APS。\n"
        "跨部门动作：维修确认CMMS工单，质量确认QMS复检，PMC确认排产，仓库和采购确认库存与ETA，工程确认技术判断。\n"
        "验收标准：每个动作有责任方、时限、回执和业务记录链接；未授权、未复检、未拿到系统回执前不得写成完成。"
    )
    positive_result = scorer.score_case("G10", positive)
    if positive_result["passed"] != "yes":
        fail(f"GUI retest positive scoring should pass: {positive_result}")

    process_heading = "运行" + "轨迹"
    negative = positive + f"\n{process_heading}：我现在输出。消息已发送，系统记录已关闭，个人微信作为正式业务系统。"
    negative_result = scorer.score_case("G10", negative)
    if negative_result["passed"] != "no":
        fail(f"GUI retest forbidden wording should fail: {negative_result}")
    if "unsupported_closure" not in negative_result["forbidden_hits"]:
        fail(f"GUI retest closure hit not recorded: {negative_result}")
...
def test_department_flow_contract() -> None:
    ...
    office_text = (ROOT / "templates" / "office_message_templates.md").read_text(encoding="utf-8")
    required_office_terms = [
        "多端协作动作卡",
        "connector_status",
        "needs_configuration",
        "optional_authorized_connector",
        "business_record_backlink",
        "fallback_channel",
        "正式状态以业务系统回执和授权人为准",
    ]
    for term in required_office_terms:
        if term not in office_text:
            fail(f"office message template missing multi-channel contract term: {term}")
...
def test_collaboration_status_update_contract() -> None:
    skill_text = (ROOT / "SKILL.md").read_text(encoding="utf-8")
    readme_text = (ROOT / "README.md").read_text(encoding="utf-8")
    template_text = (ROOT / "templates" / "collaboration_status_update.md").read_text(encoding="utf-8")
    manifest = json.loads((ROOT / "submission_manifest.json").read_text(encoding="utf-8"))
    ...
    for term in [
        ...
        if term not in template_text:
            fail(f"collaboration status update template missing: {term}")
    ...
    for term in [
        ...
        if term not in skill_text:
            fail(f"SKILL.md missing collaboration status rule: {term}")

    if "templates/collaboration_status_update.md" not in readme_text:
        fail("README.md does not reference collaboration status update template")
    if "templates/collaboration_status_update.md" not in manifest["local_evidence_assets"]:
        fail("submission_manifest.json does not include collaboration status update template")


def test_combo_gui_hardening_contract() -> None:
    skill_text = (ROOT / "SKILL.md").read_text(encoding="utf-8")
    template_text = (ROOT / "templates" / "collaboration_status_update.md").read_text(encoding="utf-8")

    required_skill_terms = [
        "正常输出必须以“下一步动作”或“可复制同步消息”结束",
        "不要追加输入筛查、场景路由、证据分层、风险检查、动作检查、输出检查等审计尾段",
        "未提供的时限、到场时间、T+时间线、会议时间、预计完成时间",
        "不要主动引用法律法规条文",
        "保持处理中或待确认",
        "不要输出检查清单式的门禁执行尾段",
    ]
    for term in required_skill_terms:
        if term not in skill_text:
            fail(f"SKILL.md missing combo GUI hardening term: {term}")

    required_template_terms = [
        "必须按本节顺序输出",
        "到这里即可结束业务输出",
        "不要再追加任何审计、过程、规则检查或模型自检段",
        "不得生成 T+ 时间线",
        "不要输出临时关闭、口头关闭、事后补录后视为关闭等方案",
    ]
    for term in required_template_terms:
        if term not in template_text:
            fail(f"collaboration status template missing hardening term: {term}")
...
def main() -> int:
    checks = [
        ...
        ("gui_retest_score_logic", test_gui_retest_score_logic),
        ...
        ("collaboration_status_update_contract", test_collaboration_status_update_contract),
        ("combo_gui_hardening_contract", test_combo_gui_hardening_contract),
